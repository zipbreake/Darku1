...
RCSTAG_CC("$Id: parse.c,v 1.1.1.1 2005/02/19 19:06:44 maqui Exp $");
...
aMessage msgtab[] = {
    ...
    {CLASS_USERS,	MSG_USERS,	TOK_USERS,	m_users,	0, MAXPARA,	MFLG_SLOW,      0L},
...
/*
** Esto hace falta porque puede ser que pasemos
** a una rutina una cadena que dicha rutina puede liberar internamente.
** Eso seria un bug.
*/
  char remitente[HOSTLEN + 1];
  ...
  if (from->name)
  {
    assert(strlen(from->name) <= HOSTLEN);
    strcpy(remitente, from->name);
    para[0] = remitente;
  }
  else
  {
  ...
  if (*ch == '\0')
  {
    ...
    Debug((DEBUG_NOTICE, "Empty message from host %s:%s",
        PunteroACadena(cptr->name), PunteroACadena(from->name)));
    return (-1);
  }
    ...
    if (buffer[0] != '\0')
    {
      if (IsUser(from))
        sendto_one(from, ":%s %d %s %s :Unknown command",
            me.name, ERR_UNKNOWNCOMMAND, PunteroACadena(from->name), ch);
...
int parse_server(aClient *cptr, char *buffer, char *bufend)
{
  ...
  if (len > 402)
  {
    ...
    sendto_ops("RCV:%-8.8s(%.4d): \"%s...%s\"",
        PunteroACadena(cptr->name), len, buffer, &buffer[len - 200]);
    buffer[200] = c;
  }
  else
    sendto_ops("RCV:%-8.8s(%.4d): \"%s\"", PunteroACadena(cptr->name), len, buffer);
  sdbflag = 0;
#endif /* GODMODE */
    ...
    *ch++ = '\0';
    ...
    if (!from)
    {
      Debug((DEBUG_NOTICE, "Unknown prefix (%s)(%s) from (%s)",
          para[0], buffer, PunteroACadena(cptr->name)));
      ircstp->is_unpf++;
      while (*ch == ' ')
        ch++;
      /*
      ...
      if (ch[1] == 'Q')
      {
        para[0] = PunteroACadena(cptr->name);
        from = cptr;
      }
      else
        return 0;
    ...
    else if (from->from != cptr)
    {
      ...
      Debug((DEBUG_NOTICE, "Fake direction: Message (%s) coming from (%s)",
          buffer, PunteroACadena(cptr->name)));
      return 0;
    }
  }
