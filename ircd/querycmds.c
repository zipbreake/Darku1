...
RCSTAG_CC("$Id: querycmds.c,v 1.1.1.1 2005/02/19 19:06:44 maqui Exp $");
...
int m_links(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (ocultar_servidores_en_el_map && !IsOper(sptr) && !IsHelpOp(sptr)) {
       	sendto_one(sptr, ":%s NOTICE %s :LINKS ha sido deshabilitado.", me.name, parv[0]);
	return 0;
  } 
