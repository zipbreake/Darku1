...
RCSTAG_CC("$Id: channel.c,v 1.6 2005-11-04 09:46:55 zipbreake Exp $");
...
int can_send(aClient *cptr, aChannel *chptr, char *msgtext)
{
  ...
  if (IsChannelService(cptr) || IsServer(cptr) || (IsServicesBot(cptr)))
    return 0;                   /* siempre pueden */

  lp = IsMember(cptr, chptr);

  flag = !lp || !(lp->flags & (CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE))
      || (lp->flags & CHFL_ZOMBIE);

  if (!lp && IsUserBot(cptr))
  ...
  if ((chptr->mode.mode & MODE_NOCOLOR) && msgtext)
	if (strchr((char *)msgtext, 3) || strchr((char *)msgtext, 2) || strchr((char *)msgtext, 31))
	      return (MODE_NOCOLOR);

  if (chptr->mode.mode & MODE_NOCTCP && msgtext && (!lp || !(lp->flags & (CHFL_CHANOP))))
  ...
  *modebuf = *parabuf = '\0';
  ...
  else
  {
    static unsigned int current_flags[8] =
        { 0, CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE, CHFL_CHANOP | CHFL_HALFOP, CHFL_CHANOP | CHFL_VOICE, CHFL_HALFOP | CHFL_VOICE, CHFL_VOICE, CHFL_HALFOP, CHFL_CHANOP };
    ...
    for (first = 1; full; first = 0)  /* Loop for multiple messages */
    {
          ...
          lp1 = chptr->members, new_mode = 1, flag_cnt++)
      {
        ...
        {
          if ((lp1->flags & (CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE)) !=
              current_flags[flag_cnt])
            continue;           /* Skip members with different flags */
          if (sblen + NUMNICKLEN + 5 > BUFSIZE - 3)
...
int m_botmode(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!(sendts = set_mode(cptr, sptr, chptr, 1, parc - 3, parv + 3,
  ...
  if (strlen(modebuf) > (size_t)1 || sendts > 0)
  {
    if (strlen(modebuf) > (size_t)1)
      sendto_channel_butserv(chptr, NULL, ":%s MODE %s %s %s",
          bot_virtual_chanserv, chptr->chname, modebuf, parabuf);
    if (IsLocalChannel(chptr->chname))
      return 0;
...
int m_mode(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!(sendts = set_mode(cptr, sptr, chptr, 0, parc - 2, parv + 2,
      modebuf, parabuf, nparabuf, &badop)))
  {
    sendto_one(sptr, err_str(IsMember(sptr, chptr) ? ERR_CHANOPRIVSNEEDED :
        ERR_NOTONCHANNEL), me.name, parv[0], chptr->chname);
...
static int DoesOp(char *modebuf)
{
  ...
  while (*++modebuf)
    if (*modebuf == 'o' || *modebuf == 'h' || *modebuf == 'v')
      return (1);
  return 0;
}
...
static int canal_flags[] = {
  ...
  MODE_NOCOLOR, 'c', MODE_ONLYOPERS, 'O',
  MODE_HALFOP, 'h',
  0x0, 0x0
};
...
/*
** jcea - 08/Abr/03
** Esta rutina devuelve una cadena (estatica)
** con los modos a activar/desactivar, listos
** para ser enviados. No cambia los modos del canal.
*/
char *adapta_y_visualiza_canal_flags(aChannel *chptr, int add, int del)
{
...
/*
** jcea - 08/Abr/03
** Esta rutina devuelve los modos "+" y "-" en la cadena
** que se le indica. Si hay modos inconsistentes ("+m-m", por ejemplo),
** el resultado es indefinido.
** En este momento no gestionamos poner modos con parametros ("+k", "+l"),
** pero puede usarse ("-k", "-l"), sin parametros.
*/
void mascara_canal_flags(char *modos, int *add, int *del)
{
  char c, *p;
  int *ip;
  int addi, deli, modo;
...
out:
  if ((addi & MODE_PRIVATE) && (addi & MODE_SECRET))
  {                             /* "+s" y "+p" a la vez */
    addi &= ~MODE_PRIVATE;      /* Nos quedamos solo con +s */
  }
  if (addi & MODE_PRIVATE)
  {
    deli |= MODE_SECRET;
  }
  if (addi & MODE_SECRET)
  ...
  deli ^= addi & deli;          /* En caso de conflicto, gana el 'add' */
  ...
  while (curr && *curr)
  {
    switch (*curr)
    {
      ...
      case 'v':
        ...
        if (whatt == MODE_DEL && IsChannelService(who) && *curr == 'o' && (who != sptr))
        {
          sendto_one(cptr, err_str(ERR_ISCHANSERVICE), me.name,
              cptr->name, parv[0], chptr->chname);
          break;
        }
        /* Siempre que un modo se quite sobre uno mismo se permite */
        if (whatt == MODE_DEL && (who == sptr))
        {
            zipbreake_emule_xmode = !0;
        }
	/* Puede darse otro caso. Un halfop de/quite voz a un usuario */
        if (*curr == 'v' && is_half_op(sptr, chptr))
        {
            zipbreake_emule_xmode = !0;
        }
      ...
      case 'k':
        ...
        if (whatt == MODE_ADD)
        {
          ...
          {
            if (!(chptr->modos_prohibidos & MODE_KEY))
        ...
        {
          if ((mode->key) && (strCasediff(mode->key, *parv) == 0))
          {
            if (!(chptr->modos_obligatorios & MODE_KEY))
            {
              lp = &chops[opcnt++];
      ...
      case 'l':
        ...
        if (whatt == MODE_DEL)
        {
          if (!(chptr->modos_obligatorios & MODE_LIMIT))
          {
            limitset = 1;
            nusers = 0;
          }
        ...
        if (--parc > 0)
        {
          ...
          if (!limit_wrong)
          {
            ...
            {
              if (atoi(*++parv) < 1)
                limit_wrong = 1;  /* Un valor invalido, quitamos el +l */
              else
                nusers = atoi(*parv); /* Un valor correcto */
            }

            if (!(nusers))
              break;

            if (!(chptr->modos_prohibidos & MODE_LIMIT))
            {
              lp = &chops[opcnt++];
      ...
      case 'i':                /* falls through for default case */
        ...
        goto def;               /* Porque insertamos codigo */
      /* El modo O solo pueden cambiarlo los Opers e ircops a voluntad */
      case 'O':
...
** jcea@argo.es - 03/Feb/98
*/
  if (!jcea_xmode_esnet && !zipbreake_emule_xmode
...
/*
** Esto habra que quitarlo en el futuro.
** Es para que la gente no juegue con modos
** aun no completamente implementados.
*/
/* Ignora el cambio si es de un usuario local */
  {
    unsigned int activacion_modos;

/*
** Aislamos los modos que se acaban de activar por el usuario,
...
*/
  ...
  newmode |= chptr->modos_obligatorios;
  newmode &= ~MODE_WPARAS;      /* Estos modos son especiales y no se guardan aqui */
      ...
      switch (lp->flags & MODE_WPARAS)
      {
        case MODE_KEY:
          if (strlen(cp) > (size_t)KEYLEN)
            *(cp + KEYLEN) = '\0';
          if ((whatt == MODE_ADD && (!(mode->key) ||
              (strCasediff(mode->key, cp) != 0))) ||
              (whatt == MODE_DEL && (mode->key)))
          {
    ...
    int parc, char *parv[], char *mbuf, char *pbuf, char *npbuf, int *badop)
{
  ...
  if (IsServer(cptr))
  {
    return set_mode_remoto(cptr, sptr, chptr, botmode, parc, parv, mbuf, pbuf,
        npbuf, badop);
  }
  else
  {
    return set_mode_local(cptr, sptr, chptr, botmode, parc, parv, mbuf, pbuf,
        npbuf, badop);
  }
}
...
*/
    if (
        ...
        reg &&
#endif
        IsNickRegistered(sptr) && (IsServicesBot(sptr) || IsHelpOp(sptr)) && (!compall("OPER", key)))
      return 0;
#endif
 ...
 */
  ...
  if (OnlyOpersChannel(chptr) && !(IsHelpOp(sptr) || IsOper(sptr)))
    return ERR_NEEDOPERSTATUS;

  /* Los UserBots no pueden entrar a los canales */
  if (IsUserBot(sptr) && !IsOper(sptr)) {
      sendto_one(sptr, ":%s NOTICE %s :JOIN esta deshabilitado para este nick.", me.name, sptr->name);
      return 0;
  }
  ...
  *jbuf = *mbuf = '\0';         /* clear both join and mode buffers -Kev */
  ...
  for (name = strtoken(&p, parv[1], ","); name; name = strtoken(&p, NULL, ","))
  {
    ...
    {                           /* not a /join 0, so treat it as
      ...
      {
/*
** No admitimos caracteres de control en el nombre del canal,
** si el que pide el "join" es un usuario local.
*/
        if (MyUser(sptr))
        {
          unsigned char *p;
          ...
          for (p = name; *p; p++)
          {
            if (*p < 32)
            {
              sendto_one(sptr, err_str(ERR_NOSUCHCHANNEL), me.name, parv[0],
                  name);
              break;            /* No podemos empezar el bucle exterior directamente */
            }
          }
          if (*p)
      ...
      if (MyConnect(sptr))
      {
        ...
        if ((!is_invited(sptr, chptr))
            && check_target_limit(sptr, chptr, chptr->chname, created))
        {
      ...
      for (name = strtoken(&p, jbuf, ","); name; name = strtoken(&p, NULL, ","))
      {
            ...
            && db_es_miembro(BDD_CHANDB_OLD, chptr->chname, sptr->name))
        {
          ...
          sendto_lowprot_butone(cptr, 9, ":%s BMODE %s %s +o %s",
              me.name, bot_virtual_chanserv, chptr->chname, sptr->name);
          sendto_highprot_butone(cptr, 10, "%s BMODE %s %s +o %s%s",
              NumServ(&me), bot_virtual_chanserv, chptr->chname, NumNick(sptr));

          /* queridos usuarios, ahora os tomo el pelo como a chinos */
          sendto_channel_butserv(chptr, NULL, ":%s MODE %s +o %s",
              bot_virtual_chanserv, chptr->chname, sptr->name);
        }
      }
    }
    ...
    int *send_itp, char is_a_ban, int mode)
{
  ...
  *sblenp += strlen(token);
                                   ...
                                   of modes for nicks: */
  {
    ...
    mode &= CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE;
    if (first)
      last_mode = 0;
...
int m_burst(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!chptr->creationtime || chptr->creationtime > timestamp)
  {
    ...
    /* Estamos en netride... ¡Quitamos el topic si existe! -- jduarter@navegalia.com */
    ...
    if (chptr->topic)
    {
      ...
      chptr->topic_nick = '\0';
      chptr->topic_time = 0;
      /* Hemos borrado todo, ahora anunciamos al canal
  ...
  *modebuf = '+';
        ...
        while (*(++p))          /* Run over all mode characters */
        {
          switch (*p)           /* which mode ? */
          {
            ...
            case 'R':
            {
              ...
              {
                bmodebuf[mblen++] = 'R';
                current_mode->mode |= MODE_REGNICKS;
              }
              if (!tmp)
                modebuf[mblen2++] = 'R';
              break;
            ...
            {
              register int tmp;
              prev_mode &= ~MODE_ONLYOPERS;
              if (!(tmp = netride ||
                  (current_mode->mode & MODE_ONLYOPERS)) || wipeout)
              {
                bmodebuf[mblen++] = 'O';
                current_mode->mode |= MODE_ONLYOPERS;
              }
              if (!tmp)
                modebuf[mblen2++] = 'O';
              break;
            ...
            case 'M':
            {
              ...
              {
                bmodebuf[mblen++] = 'M';
                current_mode->mode |= MODE_MSGNONREG;
              }
              if (!tmp)
                modebuf[mblen2++] = 'M';
              break;
            ...
            {
              register int tmp;
              prev_mode &= ~MODE_NOCOLOR;
              if (!(tmp = netride ||
                  (current_mode->mode & MODE_NOCOLOR)) || wipeout)
              {
                bmodebuf[mblen++] = 'c';
                current_mode->mode |= MODE_NOCOLOR;
              }
              if (!tmp)
                modebuf[mblen2++] = 'c';
              break;
  ...
  if (wipeout)
  {
    ...
    if ((prev_mode & MODE_NOCOLOR))
      cancel_mode(sptr, chptr, 'c', NULL, &count);
    if ((prev_mode & MODE_ONLYOPERS))
...
int m_part(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  *pbuf = '\0';                 /* Initialize the part buffer... -Kev */
  ...
  if (IsServer(sptr))           /* Sin esto no pasa nada, pero por si las moscas.. */
    return 0;
  ...
  /* Los UserBots no pueden salir de los canales */
  if (IsUserBot(sptr) && !IsOper(sptr)) {
      sendto_one(sptr, ":%s NOTICE %s :PART esta deshabilitado para este nick.", me.name, sptr->name);
      return 0;
  }
  ...
  for (; (name = strtoken(&p, parv[1], ",")); parv[1] = NULL)
  {
    ...
    if (can_send(sptr, chptr, NULL) != 0) /* Returns 0 if we CAN send */
      comment = NULL;
...
int m_kick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  *nickbuf = *buf = '\0';
  ...
  if (!IsServicesBot(sptr) && !IsServer(cptr) && !is_chan_op(sptr, chptr) && !(is_half_op(sptr,chptr) && !(is_chan_op(who,chptr))))
  {
      ...
      !(lp->flags & CHFL_ZOMBIE)) || IsServer(sptr))
  {
        ...
        ((lp2 && (lp2->flags & CHFL_DEOPPED)) || (!lp2 && IsUser(sptr))))
    {
      ...
      {
        if (Protocol(cptr) < 10)
          sendto_one(cptr, ":%s MODE %s +h %s " TIME_T_FMT,
              me.name, chptr->chname, who->name, chptr->creationtime);
        else
          sendto_one(cptr, "%s MODE %s +h %s%s " TIME_T_FMT,
              NumServ(&me), parv[1], NumNick(who), chptr->creationtime);
      }
  ...
  for (; (name = strtoken(&p, parv[1], ",")); parv[1] = NULL)
  {
    ...
    if (!topic)                 /* only asking  for topic  */
    {
      ...
      else
      {
        ...
        !(b = is_banned(sptr, chptr, NULL))) || is_chan_op(sptr, chptr) || is_half_op(sptr, chptr)
...
/*
** Si el canal *NO* existe, no nos deja invitar
** OJO con lo que sigue, ¡que lo antiguo estaba
** hecho con idea!.
*/
    if (MyConnect(sptr))
    {
  ...
  if (!IsMember(sptr, chptr) && !IsServicesBot(sptr))
  {
    sendto_one(sptr, err_str(ERR_NOTONCHANNEL), me.name, parv[0],
        chptr->chname);
    return 0;
...
int m_names(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (parc < 2)
  {
    sendto_one(sptr, err_str(ERR_NEEDMOREPARAMS), me.name, parv[0], "NAMES");
    return 0;
  }
  ...
  else
  {
    ...
    switch (mtype)
    {
      case 1:                  /* Convert nicks for MODE HACKs here  */
      {
        ...
        while (i < parc)
        {
          while (*mode && *mode != 'o' && *mode != 'h' && *mode != 'v')
            ++mode;
          strcat(params, " ");
