...
RCSTAG_CC("$Id: whocmds.c,v 1.6 2005/06/26 20:30:54 zipbreake Exp $");
    ...
    int fields, char *qrt)
{
  ...
  static char buf1[512], servername[32];
  ...
  {
    Reg3 char *p2 = servername;
    if (ocultar_servidores_en_el_map && !(IsHelpOp(sptr) || IsOper(sptr) || (sptr == acptr))) {
       sprintf(servername, "*.%s", dominio_de_la_red);
    } else {
       sprintf(servername, "%s", acptr->user->server->name);
    }
    *(p1++) = ' ';
    while ((*p2) && (*(p1++) = *(p2++)));
  }
  ...
  if (!fields || (fields & WHO_FIELD_FLA))
  {
    *(p1++) = ' ';
    ...
    if (IsDeaf(acptr))
      *(p1++) = 'd';
    if (IsOnlyPreOper(acptr))
      *(p1++) = 'p';
    if (IsOnlyOper(acptr))
      *(p1++) = 'h';
    if (IsOnlyCoAdmin(acptr))
      *(p1++) = 'c';
    if (IsAdmin(acptr))
      *(p1++) = 'a';
    if (IsHidden(acptr))
...
int m_whois(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  static char buf[512], servername[64], serverdesc[128];
  static char rpl_oper[] = "es un OPERador de los servicios de red";
  static char rpl_admin[] = "es un ADMINistrador de los servicios de red";
  static char rpl_coadmin[] = "es un CoADMINistrador de los servicios de red";
  static char rpl_preoper[] = "es un PreOPERador de los servicios de red";
  static char rpl_ircop[] = "es un IRCop del servidor";
  static char rpl_sadmin[] = "es un IRCop - Administrador del servidor";
    ...
    if (wilds)
    {
      ...
      exact_match:
            ...
            )
        {
          ...
          ocultacion_canales = ((IsServicesBot(acptr) && IsInvisible(acptr)) || IsMasInvisible(acptr) && !IsHelpOp(sptr) && !IsOper(sptr));
          canales_ocultos = 0;
        ...
        sprintf(servername, "%s", a2cptr->name);
        sprintf(serverdesc, "%s", PunteroACadena(a2cptr->info));
    	if (ocultar_servidores_en_el_map && !(IsHelpOp(cptr) || IsOper(cptr) || (cptr == acptr))) {
	   if (dominio_de_la_red)
               sprintf(servername, "*.%s", dominio_de_la_red);
	   if (descripcion_servidores)
               sprintf(serverdesc, "%s", descripcion_servidores);
        }

        sendto_one(sptr, rpl_str(RPL_WHOISSERVER), me.name,
       	    parv[0], name, servername, serverdesc);
        ...
        if (user)
        {
          ...
          if (IsAdmin(acptr))
            sendto_one(sptr, rpl_str(RPL_WHOISHELPOP), me.name, parv[0], name, rpl_admin);
          else if (IsCoAdmin(acptr))
            sendto_one(sptr, rpl_str(RPL_WHOISHELPOP), me.name, parv[0], name, rpl_coadmin);
          else if (IsHelpOp(acptr))
            sendto_one(sptr, rpl_str(RPL_WHOISHELPOP), me.name, parv[0], name, rpl_oper);
          else if (IsPreOper(acptr))
            sendto_one(sptr, rpl_str(RPL_WHOISHELPOP), me.name, parv[0], name, rpl_preoper);
          ...
          if (IsAnOper(acptr)) {
	    if (IsSAdmin(acptr))
               sendto_one(sptr, rpl_str(RPL_WHOISOPERATOR), me.name, parv[0], name, rpl_sadmin);
	    else
               sendto_one(sptr, rpl_str(RPL_WHOISOPERATOR), me.name, parv[0], name, rpl_ircop);
	  }

          if (IsMsgOnlyReg(acptr))
            sendto_one(sptr, rpl_str(RPL_MSGONLYREG), me.name, parv[0], name);

          if (MyConnect(acptr) && (!IsMasInvisible(acptr) || IsHelpOp(sptr) || IsOper(sptr) || acptr == sptr))
