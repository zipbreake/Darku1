...
RCSTAG_CC("$Id: map.c,v 1.2 2005/06/14 09:26:54 zipbreake Exp $");
    ...
    int prompt_length)
{
  ...
  *p = '\0';
  ...
  else
  {
    ...
    if (ocultar_servidores_en_el_map && !(IsHelpOp(cptr) || IsOper(cptr) || (cptr->user->server->name == server->name))) {
	if (!buscar_uline(cptr->confs, server->name)) {
	   sprintf(servername, "*.%s", dominio_de_la_red);
	   mostrar = 1;
	}
    } else {
       sprintf(servername, "%s", server->name);
       mostrar = 1;
    }

    if (mostrar)
       sendto_one(cptr, rpl_str(RPL_MAP), me.name, cptr->name, prompt,
        ((IsBurstOrBurstAck(server)) ? "*" : ""), servername, buf, lag,
...
int m_map(aClient *UNUSED(cptr), aClient *sptr, int parc, char *parv[])
{
  ...
  if ((ocultar_servidores_en_el_map > 1) && !IsOper(sptr) && !IsHelpOp(sptr)) {
        sendto_one(sptr, ":%s NOTICE %s :MAP ha sido deshabilitado.", me.name, parv[0]);
        return 0;
  }
