...
int ocultar_servidores_en_el_map;
int numero_maximo_de_clones_por_defecto;
char *dominio_de_la_red;
char *descripcion_servidores;
char *nombre_de_la_red;
char *clave_de_cifrado_de_ips;
char *bot_virtual_chanserv;
char *bot_virtual_nickserv;
char *final_ipvirtual_ubot;
  ...
  for (reg = *reg3; reg != NULL; reg = reg2)
  {
    ...
    if (!strcmp(reg->clave, c))
    {
      *reg3 = reg2;
      ...
      switch (tabla)
      {
        /* 19-Oct-05: zipbreake@darkbolt.org
        *
        * Baja de nicks de forma instantanea.
        *
        */
       case ESNET_NICKDB:
       {
        aClient *sptr;
        if ((sptr = FindUser(clave)) && MyConnect(sptr) && (IsNickRegistered(sptr) || IsNickSuspended(sptr)))
        {
	 ...
	 ClearNickSuspended(sptr);
       ...
       case BDD_OPERDB:
       {
        ...
        if ((sptr = FindUser(clave)) && MyConnect(sptr) && (IsPreOper(sptr) || IsSAdmin(sptr) || IsServicesBot(sptr)))
        {
         /* El usuario está conectado, y en nuestro servidor. */
         int of, oh;
         of = sptr->flags;
	 ...
	 if (IsServicesBot(sptr))
         ...
         ClearPreOper(sptr);
         ClearSAdmin(sptr);
	 /* Tambien debemos quitar los modos +k +X y los flags de acceso a modos */
	 ClearBDDflags(sptr);
	 ...
	 if (!IsOper(sptr))
        ...
        case BDD_CONFIGDB:
          if (!reemplazar)
          {
            ...
            else if (!strcmp(c, BDD_OCULTAR_SERVIDORES_EN_EL_MAP))
            {
              ocultar_servidores_en_el_map = 0;
            }
            else if (!strcmp(c, BDD_NUMERO_MAXIMO_DE_CLONES_POR_DEFECTO))
            {
              numero_maximo_de_clones_por_defecto = 0;
            }
            else if (!strcmp(c, BDD_DOMINIO_DE_LA_RED))
            {
              dominio_de_la_red = NULL;
            }
            else if (!strcmp(c, BDD_DESCRIPCION_SERVIDORES))
            {
              descripcion_servidores = NULL;
            }
            else if (!strcmp(c, BDD_NOMBRE_DE_LA_RED))
            {
              nombre_de_la_red = NULL;
            }
            else if (!strcmp(c, BDD_BOT_VIRTUAL_NICKSERV))
            {
	      bot_virtual_nickserv = me.name;
            }
            else if (!strcmp(c, BDD_BOT_VIRTUAL_CHANSERV))
            {
	      bot_virtual_chanserv = me.name;
            }
            else if (!strcmp(c, BDD_FINAL_IPVIRTUAL_UBOT))
            {
              final_ipvirtual_ubot = NULL;
            }
  ...
  switch (tabla)
  {
   ...
   case BDD_OPERDB:
   {
     ...
     { 
       ...
       {
        /* El usuario tiene +r debemos reasignarle los privilegios */
	set_bddflags(sptr, reg->valor);
       ...
       * Agregado también el efecto de nuevo registro (19-Oct-05: zipbreake@darkbolt.org)
       *
       */
    case ESNET_NICKDB:
    {
      ...
      if (prohibido && (sptr = FindUser(clave)) && MyConnect(sptr)) {
        sendto_one(sptr, ":%s NOTICE %s :*** Tu nick %s acaba de ser prohibido.", bot_virtual_nickserv, clave, clave);
        rename_user(sptr, NULL);
      } else if (suspendido && (sptr = FindUser(clave)) && MyConnect(sptr)) {
        int of, oh;
        of = sptr->flags;
        oh = sptr->hmodes;
	ClearNickRegistered(sptr);
	SetNickSuspended(sptr);
        send_umode_out(sptr, sptr, of, oh, IsRegistered(sptr));
        sendto_one(sptr, ":%s NOTICE %s :*** Tu nick %s acaba de ser suspendido.", bot_virtual_nickserv, clave, clave);
      } else if ((sptr = FindUser(clave)) && MyConnect(sptr)) {
        int of, oh;
        of = sptr->flags;
        oh = sptr->hmodes;
    ...
    case BDD_CONFIGDB:
      ...
      {
        bot_virtual_nickserv = v;
      }
      else if (!strcmp(c, BDD_BOT_VIRTUAL_CHANSERV))
      {
        bot_virtual_chanserv = v;
      }
      else if (!strcmp(c, BDD_FINAL_IPVIRTUAL_UBOT))
      {
        final_ipvirtual_ubot = v;
      }
      else if (!strcmp(c, BDD_CLAVE_DE_CIFRADO_DE_IPS))
      {
        char tmp, clave[12 + 1];
...
*/
  ...
  /* Asignamos un valor por defecto a los pseudobots */
  bot_virtual_nickserv = me.name;
  bot_virtual_chanserv = me.name;
...
*/
    ...
    if (db_buscar_registro(BDD_CONFIGDB, BDD_OCULTAR_SERVIDORES_EN_EL_MAP))
    {
      ocultar_servidores_en_el_map = !0;
    }
    if ((reg =
    ...
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_DOMINIO_DE_LA_RED)))
    {
      dominio_de_la_red = reg->valor;
    }
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_DESCRIPCION_SERVIDORES)))
    {
      descripcion_servidores = reg->valor;
    }
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_NOMBRE_DE_LA_RED)))
    {
      nombre_de_la_red = reg->valor;
    }
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_BOT_VIRTUAL_NICKSERV)))
    {
      if (reg->valor && (strlen(reg->valor) < HOSTLEN))
	  bot_virtual_nickserv = reg->valor;
    }
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_BOT_VIRTUAL_CHANSERV)))
    {
      if (reg->valor && (strlen(reg->valor) < HOSTLEN))
	  bot_virtual_chanserv = reg->valor;
    }
    if ((reg = db_buscar_registro(BDD_CONFIGDB, BDD_FINAL_IPVIRTUAL_UBOT)))
    {
      final_ipvirtual_ubot = reg->valor;
    }
...
*/
  ...
  switch (tabla)
  {
    case ESNET_NICKDB:         /* 'n' */
      ...
      break;
    case BDD_CONFIGDB:         /* 'z' */
      if (!strcmp(clave, BDD_CLAVE_DE_CIFRADO_DE_IPS) && (!IsAdmin(sptr)))
        nivel_helper = -1;
      break;
  }
