...
RCSTAG_CC("$Id: s_conf.c,v 1.1.1.1 2005/02/19 19:06:44 maqui Exp $");
...
aConfItem *conf_negociacion = NULL;
#endif
aGline *gline = NULL;
aGline *badchan = NULL;
aMotdItem *motd = NULL;
  ...
  for (aconf = conf; aconf; aconf = aconf->next)
  {
    ...
    if (hp)
      for (i = 0, hname = hp->h_name; hname; hname = hp->h_aliases[i++])
      {
        ...
        Debug((DEBUG_DNS, "a_il: %s->%s", PunteroACadena(sockhost), fullname));
        if (strchr(aconf->name, '@'))
        {
          strcpy(uhost, PunteroACadena(cptr->username));
          strcat(uhost, "@");
        }
        else
          *uhost = '\0';
    ...
    {
      strncpy(uhost, PunteroACadena(cptr->username), sizeof(uhost) - 2);
...
/*
 * Find the single C line and return pointer to it (from list).
 * If more than one then return NULL pointer.
 */
aConfItem *count_clines(Link *lp)
{
  Reg1 aConfItem *aconf, *cline = NULL;
...
aConfItem *find_conf_exact(char *name, char *user, char *host, int statmask)
{
  ...
  for (tmp = conf; tmp; tmp = tmp->next)
  {
    if (!(tmp->status & statmask) || !tmp->name || !tmp->host ||
        (name && strCasediff(tmp->name, name)))
      continue;
    /*
...
/*
** Esto es lento y solo sirve para comprobar
...
int initconf(int opt)
{
  ...
  while (fbgets(line, sizeof(line) - 1, file))
  {
    ...
    if (aconf->status == CONF_ME)
    {
      SlabStringAllocDup(&(me.info), aconf->name, REALLEN);
      if (portnum == 0)
        portnum = aconf->port;
    }
...
int find_kill(aClient *cptr)
{
  ...
  if (reply[0])
    sendto_one(cptr, reply, me.name, ERR_YOUREBANNEDCREEP, name);
  else if (tmp)
  {
    if (BadPtr(tmp->passwd))
      sendto_one(cptr,
    ...
    else
    {
      if (*tmp->passwd == '"')
      {
        ...
        killcomment(cptr, name, &tmp->passwd[1]);
      else
#if defined(COMMENT_IS_FILE)
        killcomment(cptr, name, tmp->passwd);
#else
  ...
  else if ((agline = find_gline(cptr, NULL)) && GlineIsActive(agline))
  {
    char buf[MAXLEN * 2];
    int longitud;
    char comment[MAXLEN * 2];

    /*
     * Comprobacion longitud de gline
     */
    strcpy(comment, agline->reason);
    buf[0] = '\0';
    sprintf(buf, ":%s %d %s " TIME_T_FMT " :%s (expires at %s).",
        me.name, ERR_YOUREBANNEDCREEP, name, agline->expire,
        comment, date(agline->expire));
    longitud = strlen(buf);
    if (longitud > MAXLEN)
    {
      /* Truncamos el comentario */
      comment[strlen(agline->reason) - (longitud - MAXLEN)] = '\0';
    }
...
int find_restrict(aClient *cptr)
{
  ...
  char *rplhold = reply, *host, *name, *username, *s;
  char rplchar = 'Y';
  int pi[2], rc = 0;
  FBFILE *file = NULL;
  ...
  for (tmp = conf; tmp; tmp = tmp->next)
  {
    if (tmp->status != CONF_RESTRICT ||
        ...
        (tmp->name && username && match(tmp->name, username)))
      continue;

    if (BadPtr(tmp->passwd))
    {
      sendto_ops("Program missing on R-line %s/%s, ignoring", username, host);
      continue;
    }
    ...
    switch (rc = fork())
    {
      ...
      case 0:
      {
        ...
        if (pi[1] != 2 && pi[1] != 1)
          close(pi[1]);
        execlp(tmp->passwd, tmp->passwd, username, host, 0);
        exit(-1);
      }
      default:
        close(pi[1]);
    ...
    *reply = '\0';
    ...
    while (fbgets(temprpl, sizeof(temprpl) - 1, file))
    {
      ...
      {
        sendto_ops("R-line %s/%s: reply too long!", username, host);
        break;
      }
    }
    fbclose(file);
  ...
  if (rc)
  {
    sendto_one(cptr, ":%s %d %s :Restriction: %s",
        me.name, ERR_YOUREBANNEDCREEP, name, reply);
    return -1;
  }
  return 0;
}
