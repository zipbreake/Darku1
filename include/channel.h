...
#define CHFL_HALFOP		0x4000  /* Channel Half-op */
...
#define MODE_NOCTCP     0x0400000
#define MODE_NOCOLOR    0x0800000
#define MODE_ONLYOPERS  0x1000000

#define RegisteredChannel(x)    ((x) && ((x)->mode.mode & MODE_REGCHAN))
#define RestrictedChannel(x)    ((x) && ((x)->mode.mode & MODE_REGNICKS))
#define OnlyOpersChannel(x)     ((x) && ((x)->mode.mode & MODE_ONLYOPERS))
 ...
 */
#define MODE_WPARAS	(MODE_CHANOP|MODE_VOICE|MODE_BAN|MODE_KEY|MODE_LIMIT|MODE_HALFOP)
...
/* channel visible */
#define ShowChannel(v,c)	(PubChannel(c) || IsMember((v),(c)) || IsOper(v) || IsHelpOp(v))
...
extern int is_half_op(aClient *cptr, aChannel *chptr);
extern int is_zombie(aClient *cptr, aChannel *chptr);
extern int has_voice(aClient *cptr, aChannel *chptr);
extern int can_send(aClient *cptr, aChannel *chptr, char *msgtext);
