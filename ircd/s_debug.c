...
RCSTAG_CC("$Id: s_debug.c,v 1.3 2005/07/27 11:42:57 zipbreake Exp $");
...
#error Valor BDD_MMAP_SIZE incorrecto. Los valores correctos son 10-99.  Ejecuta el "make config" de nuevo y reajusta el valor.
#endif
    (char)('0' + (BDD_MMAP_SIZE / 10)),
    (char)('0' + (BDD_MMAP_SIZE % 10)),
    '+',
#else
    '-',
...
#endif /* (MAXWATCH > 256) || (MAXWATCH < 96) */
#else
#error MAXWATCH no definido. Ejecuta el "make config" de nuevo.
#endif /* defined(MAXWATCH) */
    '(',
#if MAXWATCH > 99
    (char)('0' + (MAXWATCH/100) % 10),
#endif
...
#error Valor CHECK_TX_MAX_LINKS demasiado grande
#endif
#if CHECK_TS_MAX_LINKS > 99
    (char)('0' + (CHECK_TS_MAX_LINKS/100) % 10),
#endif
#if CHECK_TS_MAX_LINKS > 9
    (char)('0' + (CHECK_TS_MAX_LINKS/10) % 10),
#endif
    (char)('0' + (CHECK_TS_MAX_LINKS/1) % 10),
   ')','+',
#else
   '-',
#endif
   'C','H','2',
#ifdef no_poner_version
    '.','U','N','I','W','O','R','L','D','S','.','O','R','G',':',
#endif
    'O','2',
...
#define hzz HZ
  ...
  rup = now - me.serv->boot_timestamp;
  if (secs == 0)
    secs = 1;
