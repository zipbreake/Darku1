...
RCSTAG_CC("$Id: s_user.c,v 1.17 2005-11-07 15:40:07 zipbreake Exp $");
 ...
 */
  ...
  if (IsOper(sptr))
    ++nrof.opers;
  if (!MyUser(sptr) && IsHelpOp(sptr))  /* Sumamos para entradas REMOTAS de +h o superiores */
    ++nrof.helpers;             /* Las locales se procesan en m_nick   */
  if (!MyUser(sptr) && IsServicesBot(sptr))
    ++nrof.bots_oficiales;

  if (MyConnect(sptr))
  {
    sendto_one(sptr, rpl_str(RPL_WELCOME), me.name, nick, nombre_de_la_red ? nombre_de_la_red : NETWORK_NAME, nick);
...
#define MODESCUSER "@%+"
  ...
  sprintf(buf, "PREFIX=(ohv)%s SILENCE=%d TOPICLEN=%d WALLCHOPS WHOX",
      MODESCUSER, MAXSILES, TOPICLEN);
  sendto_one(sptr, rpl_str(RPL_ISUPPORT), me.name, nick, buf);

  sprintf(buf,
      "USERIP CPRIVMSG CNOTICE CHARMAPPING=rfc1459 NETWORK=%s",nombre_de_la_red ? nombre_de_la_red : NETWORK_NAME);
  sendto_one(sptr, rpl_str(RPL_ISUPPORT), me.name, nick, buf);
    ...
    int created)
{
  ...
  if (IsChannelService(sptr) || IsServicesBot(sptr) || IsUserBot(sptr))
    return 0;
    ...
    int parc, char *parv[], int notice)
{
      ...
      nick = strtoken(&p, NULL, ","))
  {
    ...
    if (IsChannelName(nick))
    {
      ...
      {
        if (can_send(sptr, chptr, parv[2]) == 0  /* This first: Almost never a server/service */
            || IsChannelService(sptr) || IsServicesBot(sptr) || IsServer(sptr))
        {
	...
	/* Los bots de usuario no pueden recibir mensajes privados de nicks sin +r */
        if (MyUser(sptr) && IsUserBot(acptr) && !IsNickRegistered(sptr) && !IsAnOper(sptr))
        {
          sendto_one(sptr, err_str(ERR_NONONREG), me.name, parv[0], acptr->name);
          continue;
        }
	/* Los UserBot no aceptan privados de usuarios que no esten en su canal */
        if (MyUser(sptr) && IsUserBot(acptr) && !canal_comun(sptr, acptr) && !IsAnOper(sptr))
        {
          sendto_one(sptr, ":%s NOTICE %s :No puede hablar con un bot privado que no este en canales comunes.", me.name, sptr->name);
          continue;
        }
	/* Los bots de usuario puede ser que no deban aceptar privados */
        if (MyUser(sptr) && IsUserBot(acptr) && !IsBotPrivmsg(acptr) && !IsAnOper(sptr))
        {
          sendto_one(sptr, ":%s NOTICE %s :Este bot no acepta conversaciones privadas.", me.name, sptr->name);
          continue;
        }

        /* Si te tiene silenciado */
...
int whisper(aClient *sptr, int parc, char *parv[], int notice)
{
  ...
  for (lp = chptr->members; lp; lp = lp->next)
  {
    ...
    {
      s_is_member = 1;
      if ((lp->flags & (CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE)))
        s_is_voiced = 1;
      else
        break;
      if (t_is_member)
  ...
  if (MyUser(sptr))
  {
    if (sptr->user)
    {
      ...
      for (lp = sptr->user->channel; lp; lp = lp->next)
        if (can_send(sptr, lp->value.chptr, NULL) != 0)
...
int m_oper(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if ((IsServer(cptr) || IsMe(cptr)) && !IsOper(sptr))
  {
    ...
    if (CanSetSAdmin(sptr)) {
        sptr->hmodes |= HMODE_SADMIN;
        sendto_serv_butone(cptr, ":%s MODE %s :+oA", parv[0], parv[0]);
    } else {
...
int m_umode(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!IsServer(cptr))
  {
    ...
    if (sethmodes & HMODE_UBOT)
      SetUserBot(sptr);
    else
      ClearUserBot(sptr);

    if (sethmodes & HMODE_SADMIN)
      SetSAdmin(sptr);
    else
      ClearSAdmin(sptr);
  }
  ...
  {                             /* Nick Registrado */
    /* Hay ciertos modos que podemos cambiarlos según los privilegios de la propia BDD */

    if (MyConnect(sptr))
    {
      if (!(sethmodes & HMODE_ADMIN) && IsAdmin(sptr) && !CanSetAdmin(sptr))
      	ClearAdmin(sptr);
      if (!(sethmodes & HMODE_COADMIN) && IsCoAdmin(sptr) && !CanSetCoAdmin(sptr))
      	ClearCoAdmin(sptr);
      if (!(sethmodes & HMODE_HELPOP) && IsHelpOp(sptr) && !CanSetOper(sptr))
      	ClearHelpOp(sptr);
      if (!(sethmodes & HMODE_PREOPER) && IsPreOper(sptr) && !CanSetPreOper(sptr))
      	ClearPreOper(sptr);
    }
...
/*
** El +X solo se lo pueden poner los que tengan el flag de Xmode.
*/
...
/*
** Solo se puede tener un modo de representante
*/
  if (IsAdmin(sptr)) {
	ClearCoAdmin(sptr);
	ClearHelpOp(sptr);
	ClearPreOper(sptr);
  } else if (IsOnlyCoAdmin(sptr)) {
	ClearHelpOp(sptr);
	ClearPreOper(sptr);
  } else if (IsOnlyOper(sptr)) {
	ClearPreOper(sptr);
  }
  ...
  /* Cambios de IPvirtual y contadores de representantes */
  ...
  if ((sethmodes & HMODE_COADMIN) && !IsOnlyCoAdmin(sptr)) {
    --nrof.helpers;
    cambiar_ipvirtual = 1;
  }
  if (!(sethmodes & HMODE_COADMIN) && IsOnlyCoAdmin(sptr)) {
  ...
  if ((sethmodes & HMODE_HELPOP) && !IsOnlyOper(sptr)) {
    --nrof.helpers;
    cambiar_ipvirtual = 1;
  }
  if (!(sethmodes & HMODE_HELPOP) && IsOnlyOper(sptr)) {
  ...
  if ((sethmodes & HMODE_PREOPER) && !IsOnlyPreOper(sptr))
    cambiar_ipvirtual = 1;
  if (!(sethmodes & HMODE_PREOPER) && IsOnlyPreOper(sptr))
    cambiar_ipvirtual = 1;

  if ((sethmodes & HMODE_UBOT) && !IsUserBot(sptr))
    cambiar_ipvirtual = 1;
  if (!(sethmodes & HMODE_UBOT) && IsUserBot(sptr))
    cambiar_ipvirtual = 1;
  ...
  /* En este caso seteamos modos y cambiamos la IP virtual despues porque procede */
  if (cambiar_ipvirtual) {
     if (IsHidden(sptr)) {
...
void make_virtualhost(aClient *acptr, int mostrar)
{
  ...
  enum { NO, AUTOMATICA, TABLA_BDD_IPVIRTUALDB,
...
*/
  if (IsNickRegistered(acptr))
  {
    ...
    else if (IsPreOper(acptr) || IsServicesBot(acptr))
    {
      encontramos_nueva_ip_virtual_personalizada = AUTOMATICA;
    }
...
*/
  ...
  /* No hace falta una búsqueda en la BDD en este caso la vhost es automática */
  /* Según los flags de registro en la BDD y modos activos se genera */
  if (encontramos_nueva_ip_virtual_personalizada == AUTOMATICA)
  {
    int i = 0;
    while (acptr->name[i] != 0) {
           ip_virtual_temporal[i] = toLower(acptr->name[i]);
        i++;
    }
    if (IsServicesBot(acptr)) {
	strcpy(ip_virtual_temporal + 0, "bots.");
    } else if (IsAdmin(acptr)) {
	strcpy(ip_virtual_temporal + strlen(acptr->name), ".admin.");
    } else if (IsCoAdmin(acptr)) {
	strcpy(ip_virtual_temporal + strlen(acptr->name), ".coadmin.");
    } else if (IsHelpOp(acptr)) {
	strcpy(ip_virtual_temporal + strlen(acptr->name), ".oper.");
    } else if (IsPreOper(acptr)) {
	strcpy(ip_virtual_temporal + strlen(acptr->name), ".preoper.");
    }
    if (!dominio_de_la_red)
	dominio_de_la_red = "dominio.de.la.red";
    strcat(ip_virtual_temporal + strlen(ip_virtual_temporal), dominio_de_la_red);

    SlabStringAllocDup(&(acptr->user->virtualhost), ip_virtual_temporal, HOSTLEN);
    SetIpVirtualPersonalizada(acptr);
    if (mostrar && MyUser(acptr))
      sendto_one(acptr, rpl_str(RPL_HOSTHIDDEN), me.name, acptr->name,
          acptr->user->virtualhost);
    return;
  }
 ...
 */
...

  {
    ...
    {                           /* Parche DB69 */
      ...
      /* Cuando hacemos un rename perdemos todos los Flags BDD */
      ClearBDDflags(sptr);
      ClearBotPrivmsg(sptr);
...
int m_ghost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
  ...
  if (!acptr)
  {
    sendto_one(cptr,
        ...
        bot_virtual_nickserv, parv[0], parv[1]);
    return 0;
  }
  ...
  if (!reg)
  {
    sendto_one(cptr,
        ...
        bot_virtual_nickserv, parv[0], acptr->name);
    return 0;
  }
  ...
  if (!clave_ok)
  {
    sendto_one(cptr, ":%s NOTICE %s :*** Clave incorrecta.", bot_virtual_nickserv, parv[0]);
    return 0;
  }
  ...
  sendto_highprot_butone(cptr, 10,  /* Kill our old drom outgoing servers */
      "%s KILL %s%s :Comando GHOST utilizado por %s",
...
#if !0
  if ((strlen(nick) < 2) && (!IsServer(cptr))) {
      sendto_one(sptr, err_str(ERR_INVALIDNICKNAME), me.name, parv[0], parv[1]);
      return 0;
  }
...
/* Esto hay que quitarlo en algun momento... */
  ...
  if (!IsServer(cptr))
  {
    ...
    if (clave_ok)
    {
      ...
      sendto_highprot_butone(cptr, 10,  /* Kill our old drom outgoing servers */
          "%s KILL %s%s :Sesion fantasma liberada por %s",
      ...
      sendto_one(cptr,
          ...
          bot_virtual_nickserv, acptr->name);
  ...
  if (reg && (now >= cptr->nextnick))
  {
      ...
      if (sptr->name == NULL)
      {
        ...
        if (!nick_equivalentes)
          sendto_one(cptr,
              ...
              bot_virtual_nickserv, nick);

        if (nick_suspendido)
          sendto_one(cptr,
              ":%s NOTICE %s :*** Tu nick %s está suspendido.",
              bot_virtual_nickserv, nick, nick);
      }
      hflag = '+';
    ...
    else if (nick_forbid)
    {
      sendto_one(cptr,
          ...
          bot_virtual_nickserv, nombre, nick);
    ...
    else
    {
      ...
      sendto_one(cptr,
          ...
          bot_virtual_nickserv, nombre, nick, hacer_ghost ? "!" : ":");
  ...
  else if (sptr->name)
  {
    ...
    if (IsUser(sptr))
    {
      for (lp = cptr->user->channel; lp; lp = lp->next)
        if (can_send(cptr, lp->value.chptr, NULL) == MODE_BAN)
        {
      ...
      if (clave_ok)
      {
        if (!nick_equivalentes)
          sendto_one(cptr,
              ":%s NOTICE %s :*** Contraseña aceptada. Bienvenid@ a casa ;)",
              bot_virtual_nickserv, sptr->name);

        if (nick_suspendido)
          sendto_one(cptr,
              ":%s NOTICE %s :*** Tu nick %s está suspendido.",
              bot_virtual_nickserv, sptr->name, nick);
      }
    ...
    if (IsUser(sptr))
    {
	  ...
	  /* Eliminamos todos los flags BDD */
	  ClearBDDflags(sptr);
          ClearBotPrivmsg(sptr);
...
*/
  ...
  if (!nick_equivalentes)
  {
    ...
    if (hflag == '+')
    {
      ...
      if ((reg = db_buscar_registro(BDD_OPERDB, nick)) && !nick_suspendido)
      {
#if defined(BDD_VIP)
	if (!IsHidden(sptr))
	      SetHidden(sptr);
#endif
	/* Asignaremos privilegios según la propia BDD (tabla o) */
	set_bddflags(sptr, reg->valor);
      } 
      else
      {
        ClearHelpOp(sptr);
	...
	/* Eliminamos todos los flags BDD */
        ClearBDDflags(sptr);
	ClearBotPrivmsg(sptr);
	if ((reg = db_buscar_registro(BDD_BOTSDB, nick)) && !nick_suspendido) {
	     SetUserBot(sptr);
	     // Ademas el bot debe hacer auto-join a los canales permitidos
	     // y fijarse los flags que se le estimen oportunos para funcionar.
	     configure_ubot(sptr, reg->valor);
	}
      }
    }
    else
 ...
 * set_bddflags
 *
 * Recalcula los flags de BDD que debemos tener
 *
 */
int set_bddflags(aClient *sptr, char *flags)
{
    int activo = 1;

    /* Antes de fijar los flags eliminamos todo aquel que pueda existir */
    ClearBDDflags(sptr);
    ClearBotPrivmsg(sptr);
    while (*flags) {
	switch (*flags++) {
 ...
 * canal_comun
 *
 * Nos informa de si dos usuarios tienen o no un canal comun.
 *
 */
int canal_comun(aClient *sptr, aClient *acptr)
{
    return 1;
}
 ...
 * configure_ubot
 *
 * Debemos asignar la configuración de bot de la BDD al propio bot
 * Entra en los canales para los que ha sido fijado que debe hacerlo.
 *
 */
int configure_ubot(aClient *sptr, char *config)
{
  char *canal;
  char c;
